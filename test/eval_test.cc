...
TEST_CASE("Evaluation functions work properly") {
  ...
  SECTION("Mobility calculations are correct") {
    board.setToStartPos();
    REQUIRE(Eval::evaluateMobility(board, WHITE) == 20);
    REQUIRE(Eval::evaluateMobility(board, BLACK) == 20);

    board.setToFen("7k/8/4b3/8/3N4/8/4P3/K7 w - -");
    REQUIRE(Eval::evaluateMobility(board, WHITE) == 12);
    REQUIRE(Eval::evaluateMobility(board, BLACK) == 14);

    board.setToFen("7k/8/8/8/4q3/8/8/K7 w - -");
    REQUIRE(Eval::evaluateMobility(board, WHITE) == 3);
    REQUIRE(Eval::evaluateMobility(board, BLACK) == 30);

    board.setToFen("7k/8/8/3Rr3/8/8/8/K7 w - -");
    REQUIRE(Eval::evaluateMobility(board, WHITE) == 14);
    REQUIRE(Eval::evaluateMobility(board, BLACK) == 14);
  }
